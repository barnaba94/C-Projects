...
void PQEnQueue(PQueue* Que, int nKey, double prior)
{
  if ( PQisFull( Que ) )
  {
  ...
  Que -> PQueue[ Que -> PQCurrSize ].nKey = nKey;
  Que -> PQueue[ Que -> PQCurrSize ].prior = prior;
...
void DecreaseKey(PQueue* PQueue, int nKey, double nPrior) // (nPrior < PQItem.nPrior)
{
...
void IncreaseKey( PQueue* PQueue, int nKey, double nPrior ) // (nPrior > PQItem.nPrior)
{
  for (int i = 0; i < PQueue -> PQCurrSize; i++)
  {
    if ( PQueue -> PQueue[ i ] . nKey == nKey && PQueue -> PQueue[ i ].prior < nPrior )
